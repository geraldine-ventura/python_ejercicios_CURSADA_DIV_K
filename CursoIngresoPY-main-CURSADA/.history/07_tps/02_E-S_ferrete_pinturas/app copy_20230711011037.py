...
class App(customtkinter.CTk):
    def __init__(self):
        ...
        self.btn_convertir_c_f = customtkinter.CTkButton(
            master=self,
            text="Convertir °C a °F",
            command=self.btn_convertir_c_f_on_click,
        )
        self.btn_convertir_c_f.grid(row=3, pady=10, columnspan=2, sticky="nsew")

        self.btn_convertir_f_c = customtkinter.CTkButton(
            master=self,
            text="Convertir °F a °C",
            command=self.btn_convertir_f_c_on_click,
        )
    ...
    def btn_convertir_c_f_on_click(self):
        ...
        temp_cen_a_far = (temp_centigrados * 9 / 5) + 32
        mensaje = "La tempetratura ingresada expresada en °F es : " + str(
            temp_cen_a_far
        )
        alert(title="a F", message=mensaje)

    def btn_convertir_f_c_on_click(self):
        temp_fahrenheit = float(self.txt_temperatura_f.get())
        temp_far_a_cen = (temp_fahrenheit - 32) * 5 / 9
        mensaje = "La tempetratura ingersada expresada en °C es : " + str(
            temp_far_a_cen
        )
        alert(title="a C", message=mensaje)
